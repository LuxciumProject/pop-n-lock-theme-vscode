...
export const detectConfigChanges = () => {
  ...
  try {
    ...
    const reloadRequired = configRequiresReload.some(
      r => getObjectPropertyValue(changes.updatedConfigs, r) !== undefined
    );
...
const compareConfigs = (
  configs: string[]
): { updatedConfigs: IconJsonOptions; updatedJSONConfig: IconJsonOptions } => {
  ...
  return configs.reduce(
    (result, configName) => {
      try {
        ...
        const configValue =
          themeConfig.globalValue !== undefined
